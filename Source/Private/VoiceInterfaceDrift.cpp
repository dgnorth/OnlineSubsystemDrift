...
FOnlineVoiceDrift::FOnlineVoiceDrift(IOnlineSubsystem* InOnlineSubsystem) :
	OnlineSubsystem(InOnlineSubsystem),
	VoiceEngine(NULL)
{
}

FOnlineVoiceDrift::~FOnlineVoiceDrift()
{
	LocalTalkers.Empty();
	RemoteTalkers.Empty();
	VoiceEngine = NULL;
...
bool FOnlineVoiceDrift::Init()
{
	...
	{
		VoiceEngine = MakeShareable(new FVoiceEngineDrift(OnlineSubsystem));
...
void FOnlineVoiceDrift::Shutdown()
{
...
void FOnlineVoiceDrift::ClearVoicePackets()
{
	for (uint32 Index = 0; Index < MAX_SPLITSCREEN_TALKERS; Index++)
	{
		FVoicePacketDrift& LocalPacket = VoiceData.LocalPackets[Index];
...
void FOnlineVoiceDrift::Tick(float DeltaTime) 
{
...
void FOnlineVoiceDrift::StartNetworkedVoice(uint8 LocalUserNum)
{
...
void FOnlineVoiceDrift::StopNetworkedVoice(uint8 LocalUserNum)
{
	// Validate the range of the entry
	if (LocalUserNum >= 0 && LocalUserNum < MaxLocalTalkers)
	{
...
bool FOnlineVoiceDrift::RegisterLocalTalker(uint32 LocalUserNum) 
{
...
void FOnlineVoiceDrift::RegisterLocalTalkers()
{
...
bool FOnlineVoiceDrift::UnregisterLocalTalker(uint32 LocalUserNum) 
{
...
void FOnlineVoiceDrift::UnregisterLocalTalkers()
{
...
bool FOnlineVoiceDrift::RegisterRemoteTalker(const FUniqueNetId& UniqueId) 
{
	uint32 Return = ONLINE_FAIL;
	if (OnlineSubsystem)
	{
...
bool FOnlineVoiceDrift::UnregisterRemoteTalker(const FUniqueNetId& UniqueId) 
{
	uint32 Return = ONLINE_FAIL;
	if (OnlineSubsystem)
	{
...
void FOnlineVoiceDrift::RemoveAllRemoteTalkers()
{
...
FRemoteTalker* FOnlineVoiceDrift::FindRemoteTalker(const FUniqueNetId& UniqueId)
{
	for (int32 Index = 0; Index < RemoteTalkers.Num(); Index++)
	{
...
bool FOnlineVoiceDrift::IsHeadsetPresent(uint32 LocalUserNum) 
{
	return VoiceEngine.IsValid() && VoiceEngine->IsHeadsetPresent(LocalUserNum);
}

bool FOnlineVoiceDrift::IsLocalPlayerTalking(uint32 LocalUserNum) 
{
	return VoiceEngine.IsValid() && VoiceEngine->IsLocalPlayerTalking(LocalUserNum);
}

bool FOnlineVoiceDrift::IsRemotePlayerTalking(const FUniqueNetId& UniqueId) 
{
	return VoiceEngine.IsValid() && VoiceEngine->IsRemotePlayerTalking(UniqueId);
}

bool FOnlineVoiceDrift::IsMuted(uint32 LocalUserNum, const FUniqueNetId& UniqueId) const
{
...
bool FOnlineVoiceDrift::MuteRemoteTalker(uint8 LocalUserNum, const FUniqueNetId& PlayerId, bool bIsSystemWide)
{
	uint32 Return = ONLINE_FAIL;
	if (LocalUserNum >= 0 && LocalUserNum < MaxLocalTalkers )
	{
...
bool FOnlineVoiceDrift::UnmuteRemoteTalker(uint8 LocalUserNum, const FUniqueNetId& PlayerId, bool bIsSystemWide)
{
	uint32 Return = ONLINE_FAIL;
	if (LocalUserNum >= 0 && LocalUserNum < MaxLocalTalkers )
	{
...
{
	return MakeShareable(new FVoiceEngineDrift(OnlineSubsystem));
}
...
void FOnlineVoiceDrift::UpdateMuteListForLocalTalker(int32 TalkerIndex, APlayerController* PlayerController)
{
...
TSharedPtr<FVoicePacket> FOnlineVoiceDrift::SerializeRemotePacket(FArchive& Ar)
{
	TSharedPtr<FVoicePacketDrift> NewPacket = MakeShareable(new FVoicePacketDrift());
...
TSharedPtr<FVoicePacket> FOnlineVoiceDrift::GetLocalPacket(uint32 LocalUserNum)
{
	...
	{
		FVoicePacketDrift& VoicePacket = VoiceData.LocalPackets[LocalUserNum];
		if (VoicePacket.GetBufferSize() > 0)
		{
			return MakeShareable(new FVoicePacketDrift(VoicePacket));
		}
	}
...
void FOnlineVoiceDrift::ProcessTalkingDelegates(float DeltaTime)
{
...
void FOnlineVoiceDrift::ProcessLocalVoicePackets()
{
	if (VoiceEngine.IsValid())
	{
					...
					if (SpaceAvail > 0)
					{
						...
						if (Result == ONLINE_SUCCESS)
						{
							if (LocalTalkers[Index].bHasNetworkedVoice && SpaceAvail <= MAX_VOICE_PACKET_SIZE_IMPL)
							{
								...
								{
									VoiceData.RemotePackets.Add(MakeShareable(new FVoicePacketDrift(VoiceData.LocalPackets[Index])));
								}
#endif
							}
							else
...
void FOnlineVoiceDrift::ProcessRemoteVoicePackets()
{
	...
	{
		TSharedPtr<FVoicePacketDrift> VoicePacket = StaticCastSharedPtr<FVoicePacketDrift>(VoiceData.RemotePackets[Index]);
...
FString FOnlineVoiceDrift::GetVoiceDebugState() const
{
